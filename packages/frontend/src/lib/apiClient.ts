import type {
  ...
  GoalDecisionAnswerInput,
  GoalDecisionAnswerBatchInput,
...
export async function createGoalPlanningWorkflow(
  ...
  const response = await fetch(
    apiUrl(goalApiPath(goalKey, projectKey, '/planning-requests/workflows')),
    {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify(input),
