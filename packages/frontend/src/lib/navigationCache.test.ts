...
test('hydrates only exact shell and Goal projection identities with their canonical age', () => {
  ...
  const routeKey = ['mvp-goal', 'project-a', 'goal-a', 'route'] as const
  ...
  source.setQueryData(routeKey, { goal: 'goal-a' }, { updatedAt: savedAt })
  ...
  expect(target.getQueryData(routeKey)).toEqual({ goal: 'goal-a' })
  expect(target.getQueryData(unrelatedKey)).toBeUndefined()
  expect(target.getQueryState(routeKey)?.dataUpdatedAt).toBe(savedAt)
})
...
test('drops expired projections instead of presenting them as current navigation state', () => {
  ...
  const source = new QueryClient()
  const routeKey = ['mvp-goal', 'project-a', 'goal-a', 'route'] as const
  source.setQueryData(routeKey, { goal: 'old' }, { updatedAt: 1 })
  const query = source.getQueryCache().find({ queryKey: routeKey, exact: true })
  ...
  expect(target.getQueryData(routeKey)).toBeUndefined()
})
...
test('persists successful navigation reads without widening the cache scope', () => {
  ...
  expect(target.getQueryData(unrelatedKey)).toBeUndefined()
  expect(isNavigationQueryKey(['mvp-goal', 'project-a', 'goal-b', 'route'])).toBe(true)
  expect(isNavigationQueryKey(['mvp-goal', 'project-a', undefined, 'route'])).toBe(false)
})
