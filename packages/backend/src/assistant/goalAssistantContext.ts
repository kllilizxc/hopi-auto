...
\`\`\`json
{
  ...
  "actions": [
    ...
    },
    {
      ...
      "sourceResponseFormat": "optional literal 'labeled_sections', 'ordered_items', 'ordered_blocks', 'question_blocks', 'question_spans', 'inline_topics', 'topic_sentences', 'topic_spans', 'topic_closing_spans', 'topic_paragraphs', or 'topic_blocks' when sourceResponse should be interpreted as labeled answers, ordered reply items, ordered reply blocks, question-answer blocks, inline question-answer spans, inline topic clauses, sentence-level topic mentions, anchored topic sentence spans with continuation sentences, topic-closing sentence spans whose final sentence names the topic, paragraph-level topic mentions, or anchored topic blocks with continuation paragraphs",
    ...
    },
    {
      ...
      "inferOpenDecisions": "optional boolean; when true with sourceResponseFormat labeled_sections, ordered_items, ordered_blocks, question_blocks, question_spans, inline_topics, topic_sentences, topic_spans, topic_closing_spans, topic_paragraphs, or topic_blocks, runtime also resolves matching current open decisions you did not repeat in answers[]",
      "inferDecisionTopics": "optional boolean; when true with sourceResponseFormat labeled_sections, inline_topics, question_blocks, question_spans, topic_sentences, topic_spans, topic_closing_spans, topic_paragraphs, or topic_blocks, runtime also turns remaining unclaimed labeled sections, inline topic clauses, question blocks, question spans, topic sentence spans, topic-closing sentence spans, topic paragraphs, or topic blocks into durable decision topics",
      "sourceResponseFormat": "optional literal 'labeled_sections', 'ordered_items', 'ordered_blocks', 'question_blocks', 'question_spans', 'inline_topics', 'topic_sentences', 'topic_spans', 'topic_closing_spans', 'topic_paragraphs', or 'topic_blocks' when sourceResponse should be interpreted as labeled answers, ordered reply items, ordered reply blocks, question-answer blocks, inline question-answer spans, inline topic clauses, sentence-level topic mentions, anchored topic sentence spans with continuation sentences, topic-closing sentence spans whose final sentence names the topic, paragraph-level topic mentions, or anchored topic blocks with continuation paragraphs",
      ...
      "followThrough": {
        ...
        "inferRemainingAnswers": "optional boolean; when true with sourceResponseFormat question_blocks, question_spans, topic_sentences, topic_spans, topic_closing_spans, topic_paragraphs, or topic_blocks, runtime also captures the remaining unclaimed planner answers directly onto this shared follow-through",
        "answers": [
          {
...
- When current Goal state already contains the relevant open durable decisions and one structured reply answers them directly, prefer "record_answers" with "inferOpenDecisions": true plus root "sourceResponseFormat": "labeled_sections", "ordered_items", "ordered_blocks", "question_blocks", "question_spans", "inline_topics", "topic_sentences", "topic_spans", "topic_closing_spans", "topic_paragraphs", or "topic_blocks" instead of repeating those same decision topics again inside "answers".
- When there is no existing durable decision surface yet but one structured reply already names the durable decision topics, prefer "record_answers" with "inferDecisionTopics": true plus root "sourceResponseFormat": "labeled_sections", "inline_topics", "question_blocks", "question_spans", "topic_sentences", "topic_spans", "topic_closing_spans", "topic_paragraphs", or "topic_blocks" so runtime can create those durable decision topics from the remaining unclaimed topic pairs after planner-only answers are reserved.
