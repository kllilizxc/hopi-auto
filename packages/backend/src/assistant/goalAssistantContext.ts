...
\`\`\`json
{
  ...
  "actions": [
    ...
    },
    {
      ...
      "sourceResponseFormat": "optional literal 'labeled_sections', 'ordered_items', 'ordered_blocks', 'question_blocks', 'inline_topics', 'topic_sentences', 'topic_paragraphs', or 'topic_blocks' when sourceResponse should be interpreted as labeled answers, ordered reply items, ordered reply blocks, question-answer blocks, inline topic clauses, sentence-level topic mentions, paragraph-level topic mentions, or anchored topic blocks with continuation paragraphs",
    ...
    },
    {
      ...
      "inferOpenDecisions": "optional boolean; when true with sourceResponseFormat labeled_sections, ordered_items, ordered_blocks, question_blocks, inline_topics, topic_sentences, topic_paragraphs, or topic_blocks, runtime also resolves matching current open decisions you did not repeat in answers[]",
      "inferDecisionTopics": "optional boolean; when true with sourceResponseFormat labeled_sections, inline_topics, or question_blocks, runtime also turns remaining unclaimed labeled sections, inline topic clauses, or question blocks into durable decision topics",
      "sourceResponseFormat": "optional literal 'labeled_sections', 'ordered_items', 'ordered_blocks', 'question_blocks', 'inline_topics', 'topic_sentences', 'topic_paragraphs', or 'topic_blocks' when sourceResponse should be interpreted as labeled answers, ordered reply items, ordered reply blocks, question-answer blocks, inline topic clauses, sentence-level topic mentions, paragraph-level topic mentions, or anchored topic blocks with continuation paragraphs",
...
- When current Goal state already contains the relevant open durable decisions and one structured reply answers them directly, prefer "record_answers" with "inferOpenDecisions": true plus root "sourceResponseFormat": "labeled_sections", "ordered_items", "ordered_blocks", "question_blocks", "inline_topics", "topic_sentences", "topic_paragraphs", or "topic_blocks" instead of repeating those same decision topics again inside "answers".
- When there is no existing durable decision surface yet but one structured reply already names the durable decision topics, prefer "record_answers" with "inferDecisionTopics": true plus root "sourceResponseFormat": "labeled_sections", "inline_topics", or "question_blocks" so runtime can create those durable decision topics from the remaining unclaimed topic pairs after planner-only answers are reserved.
