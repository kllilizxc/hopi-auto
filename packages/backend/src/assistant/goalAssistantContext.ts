...
- When one root "answerSources" bundle already contains extra reusable snippets that do not need per-item mapping onto known pending consumers, give those remaining source entries either explicit "summary" authority or one canonical "prompt" like "What should the pilot scope be?", then prefer "sourceResponseFormat": "pending_answer_sources" or "matching_answer_sources" plus "inferDecisionTopics": true or "followThrough.inferRemainingAnswers": true over repeating those same summaries again inside "answers" or "followThrough.answers"; runtime requires leftover source entries to carry either explicit "summary" or a canonical prompt for this path.
