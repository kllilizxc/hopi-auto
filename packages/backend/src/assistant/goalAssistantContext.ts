...
- When one root "answerSources" bundle already contains extra reusable snippets that do not need per-item mapping onto known pending consumers, give those remaining source entries either explicit "summary" authority, one stable "prompt" authority, or exactly one stable "matchHint" authority. Canonical noun-phrase prompts like "What should the pilot scope be?", explicit question-shaped prompts like "How should rollout happen?", and one stable phrase hint like "launch shape" all work; runtime only accepts the "matchHints" path when there is exactly one stable hint, so it never has to guess between multiple hints. Then prefer "sourceResponseFormat": "pending_answer_sources" or "matching_answer_sources" plus "inferDecisionTopics": true or "followThrough.inferRemainingAnswers": true over repeating those same summaries again inside "answers" or "followThrough.answers".
