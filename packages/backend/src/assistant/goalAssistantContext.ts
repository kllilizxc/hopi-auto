...
- When one shared "sourceResponse" or reusable "answerSources" bundle already fits one current deterministic interpretation surface and you do not need to force a specific one, prefer root "sourceResponseFormat": "auto" so runtime can choose the strongest successful existing surface by fixed authority priority instead of hard-coding a narrower parser shape. Use an explicit concrete format only when the intended interpretation must be pinned.
