...
- When one reply is already written as natural sentences that name the topic either as a leading phrase, a prefixed phrase, or later in the sentence, like "Auth strategy should use Bun-native auth.", "For auth strategy, use Bun-native auth.", or "Use Bun-native auth for auth strategy.", prefer root "sourceResponseFormat": "topic_sentences" so runtime can match one sentence per known or inferred topic without forcing inline labels or ordered bullets.
- When one reply is already written as topic-specific paragraphs where one sentence in the paragraph names the topic, whether as a leading phrase like "Pilot scope should ...", a prefixed phrase like "About pilot scope, ...", or a trailing phrase like "... for pilot scope", prefer root "sourceResponseFormat": "topic_paragraphs" so runtime can reuse the whole paragraph for one known or inferred topic without repeating the topic name in every sentence.
- When one reply is already written as topic-specific blocks where the first paragraph names the topic and later continuation paragraphs stay on that same topic until the next anchor paragraph appears, prefer root "sourceResponseFormat": "topic_blocks" so runtime can reuse the whole anchored block whether that first paragraph uses a leading topic phrase, a prefixed topic phrase, or a trailing topic mention.
