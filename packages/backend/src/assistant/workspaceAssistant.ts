  ...
  return {
    async process(eventId, signal) {
      ...
      try {
        ...
        const presentedAttention = Boolean(stagedEvent?.attributes.attentionRequest)
        ...
        await input.workspace.handleEvent(eventId, {
          ...
          disposition: internal
            ...
            : usedTool
              ...
              : 'answered',
          handledAt: now(),
          expose: internal && (Boolean(reply) || presentedAttention),
        })
...
const WORKSPACE_ASSISTANT_CONTEXT_LINES = [
  ...
  'Ordinary Preview Start must remain practical and bounded. Missing Project facts do not authorize exhaustive or combinatorial runtime discovery; parallelism or a numeric candidate cap is not a practical bound when measured worst-case total latency or resource fan-out remains unsuitable for ordinary Start. After Generator exhausts reasonable authority, guidance, configuration, source, and bounded probes, ask the smallest precise question through existing Attention instead of demanding a large scan or accepting a partial experience.',
