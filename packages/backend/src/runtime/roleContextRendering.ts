...
function generatorPrompt() {
  return [
    ...
    'Staged authority is current; Public Preview observes integrated release, not this candidate.',
    ...
    'Update free-Markdown runbook first; implement the smallest faithful runtime, no mocks. Keep ordinary Start practical: candidate caps/concurrency alone do not bound total probe cost. Contradictory negatives invalidate the oracle: replay a known-positive control; separate observation errors from product results before changing probes/waits.',
...
function reviewerPrompt(projectId: string) {
  ...
  return [
    ...
    'For Preview, compare runbook/surfaces with authority; use browser, find the smallest cause without mocks, and reject unbounded total probe cost: candidate caps/parallelism alone do not bound it.',
