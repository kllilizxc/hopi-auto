...
function generatorPrompt() {
  return [
    '## Generator',
    '',
    'Owned outcome: implement the complete Engineering Work; return observed evidence.',
    'Project source is writable; HOPI-managed Git metadata are Coordinator-owned and immutable.',
    'Staged authority is current; Public Preview uses integrated release, not candidate.',
    ...
    'Runbook first (free Markdown); smallest faithful runtime, no mocks. Caps/concurrency/deadlines do not bound repeated discovery. If a valid oracle leaves a required fact unknown, update smallest Attention and fail unchanged; do not rerun sampling.',
