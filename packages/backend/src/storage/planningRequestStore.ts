...
import {
  ...
  normalizePlanningRequestAnswerKey as normalizeGoalPlanningRequestAnswerKey,
  normalizePlanningRequestAnswerMatchHints as normalizeGoalPlanningRequestMatchHints,
  normalizePlanningRequestAnswerSummaryKey as normalizeGoalPlanningRequestSummaryKey,
...
function mergePlanningRequestAnswers(
  ...
  return mergeSharedPlanningRequestAnswers(existing, incoming, {
    resolveInitialPrompt: resolveGoalPlanningRequestAnswerPrompt,
  })
}

function resolveGoalPlanningRequestAnswerPrompt(value: PlanningRequestAnswerValue) {
  return resolveCanonicalPromptFromSummary({
...
function normalizeLegacyPlanningRequestStatusAlias(
  ...
  if (
    LEGACY_OPEN_PLANNING_REQUEST_STATUS_ALIASES.has(normalized as (typeof TASK_STATUSES)[number])
  ) {
    return 'open'
  }
