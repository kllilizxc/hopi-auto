...
describe('RoleContextStager', () => {
  test('stages immutable authority and silently exposes source when AGENTS.md is missing', async () => {
    ...
    expect(prompt).toContain('instead of duplicating its validator')
  ...
  test('states the Git, Attention, and Run-scoped runtime boundaries for Engineering passes', async () => {
    ...
    expect(generatorPrompt).toContain('Permission follows resource ownership')
    expect(generatorPrompt).toContain('owns HOPI-managed task Git metadata')
    expect(generatorPrompt).toContain('Run-owned clone under $HOPI_RUN_SCRATCH')
    expect(generatorPrompt).toContain('Git staging, commits, branch changes, rebases, and pushes')
    expect(generatorPrompt).toContain('Do not merge, deploy, mutate production data')
