...
describe('RoleContextStager', () => {
  test('stages immutable authority and silently exposes source when AGENTS.md is missing', async () => {
    ...
    expect(prompt).toContain('single canonical owner and one-way derivation')
    expect(prompt).toContain('closed accepted schema or another finite verification oracle')
