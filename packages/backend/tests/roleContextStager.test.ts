...
describe('RoleContextStager', () => {
  ...
  test('states the Git, Attention, and Run-scoped runtime boundaries for Engineering passes', async () => {
    ...
    expect(generatorPrompt).toContain(
      'explore guidance, knowledge, behavior, source, and runbook before edits',
    )
    ...
    expect(generatorPrompt).toContain('Update/create free-Markdown runbook first')
    expect(generatorPrompt).toContain('smallest faithful runtime')
    expect(generatorPrompt).toContain('without mocks/substitute pages')
    expect(generatorPrompt).toContain('Preview Start uses bounded probes')
    expect(generatorPrompt).toContain('parallelism is not a bound')
    ...
    expect(reviewerPrompt).toContain('Use the browser')
    expect(reviewerPrompt).toContain(
      'current Goal, intended-experience authority, and Engineering Work contract',
    )
    expect(reviewerPrompt).toContain('compare runbook/surfaces with Goal')
    expect(reviewerPrompt).toContain('candidate cannot replace them')
