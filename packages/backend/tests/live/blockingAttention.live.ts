...
try {
  ...
  const initialRequest = await waitForValue(
    async () => {
      const [state, feed] = await Promise.all([
        requestJson<LiveState>(harness?.baseUrl ?? '', '/api/state'),
        requestJson<FeedView>(
          harness?.baseUrl ?? '',
          `/api/assistant/feed?projectId=${encodeURIComponent(PROJECT_ID)}&limit=100`,
        ),
      ])
      const event = feed.items.find((item) => item.event?.body.trim() === instruction)?.event
      const attention = state.attentions.find(
        (item) =>
          item.projectId === PROJECT_ID && item.resolvedAt === null && item.target === undefined,
      )
      const request =
        event && attention
          ? feed.requests.find(
              (item) =>
                ...
                item.attentions.some((candidate) => candidate.id === attention.id),
            )
          : undefined
      return { event, attention, request }
    },
    (value) =>
      value.event?.status === 'handled' &&
      Boolean(value.event.reply?.trim()) &&
      Boolean(value.attention && value.request),
    { timeoutMs: 3 * 60_000, description: 'one Project Attention presented through NeedsYou' },
  )
...
Never infer a value from existing tests or implementation. Ask the operator before Planning or
Engineering selects a value. After a durable answer, implement only that choice and run \`bun test\`
before delivery.
`,
  )
  await Bun.write(
