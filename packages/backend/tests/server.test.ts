...
`,
    ...
    const firstAttachmentAssetPath = firstAttachment.assetPath
    ...
    expect(result.actionResults).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          kind: 'request_decision',
          decision: expect.objectContaining({
            attachments: [expect.objectContaining({ assetPath: firstAttachmentAssetPath })],
          }),
        }),
        expect.objectContaining({
          kind: 'request_planning',
    ...
    const encodedAssetPath = firstAttachmentAssetPath
      .slice('assets/'.length)
      .split('/')
      .map((segment) => encodeURIComponent(segment))
      .join('/')
    const assetResponse = await server.fetch(
      new Request(`http://127.0.0.1/api/goals/test/assets/${encodedAssetPath}`),
    )
    ...
    expect(evidence.assetPath).toBe(firstAttachmentAssetPath)
