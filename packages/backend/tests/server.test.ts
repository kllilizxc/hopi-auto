...
describe('createServer', () => {
  ...
  test('materializes multiple pending planner answers through the direct planning-request API from ordered pending answer sources by durable answerKey with contiguous merge', async () => {
    const workspaceRoot = rootDir()
    const server = startServer(undefined, workspaceRoot)
  ...
  test('infers remaining planner answers through the direct planning-request API from contiguous matching answer sources by durable answerKey', async () => {
    const workspaceRoot = rootDir()
    const server = startServer(undefined, workspaceRoot)
  ...
  test('records multiple durable answers through the API from grouped named answer sources', async () => {
    const workspaceRoot = rootDir()
    const server = startServer(undefined, workspaceRoot)
  ...
  test('records multiple durable answers through the API from matching opening runs without explicit topic or question anchors', async () => {
    const workspaceRoot = rootDir()
    const server = startServer(undefined, workspaceRoot)
  ...
  test('records multiple durable answers through the API from matching closing runs without explicit topic or question anchors', async () => {
    const workspaceRoot = rootDir()
    const server = startServer(undefined, workspaceRoot)
  ...
  test('records new durable decision topics through the API from contiguous remaining matching answer sources by explicit decisionKey', async () => {
    const workspaceRoot = rootDir()
    const server = startServer(undefined, workspaceRoot)
    await createDecisionStore(workspaceRoot).createDecision('test', {
      decisionKey: 'auth-strategy',
  ...
  test('records pending open decisions through the API from ordered pending answer sources by explicit decisionKey with contiguous merge', async () => {
    const workspaceRoot = rootDir()
    const server = startServer(undefined, workspaceRoot)
  ...
  test('records new durable decision topics and inferred planner answers through the API from mixed remaining pending answer sources with explicit route metadata', async () => {
    const workspaceRoot = rootDir()
    const server = startServer(undefined, workspaceRoot)
  ...
  test('records new durable decision topics and inferred planner answers through the API from mixed remaining matching answer sources with explicit route metadata', async () => {
    const workspaceRoot = rootDir()
    const server = startServer(undefined, workspaceRoot)
  ...
  test('returns HTTP 400 when answer-driven interpretation omits both item answers and sourceResponse', async () => {
    ...
    await expect(response.json()).resolves.toMatchObject({
      error:
        'Missing answer text for decision answer auth-strategy. Provide item.answer, answerSourceKey, answerSourceGroupKey, or sourceResponse.',
    })
  })
...
`,
    ...
    const beforeResponse = await server.fetch(new Request('http://127.0.0.1/api/preferences'))
...
`,
  ...
  test('runs the configured Goal assistant and reuses grouped named answer sources across decision and planner answers', async () => {
    const workspaceRoot = await initGitRepo(rootDir())
    await writeAdapterConfig(workspaceRoot, {
      version: 1,
      assistant: {
        cmd: [
          'bun',
          '-e',
          "const [promptFile, outcomeFile] = process.argv.slice(1); const prompt = await Bun.file(promptFile).text(); if (!prompt.includes('Use grouped named answer sources across decisions and planner answers.')) throw new Error('missing user message'); await Bun.write(outcomeFile, JSON.stringify({ message: 'I reused grouped named answer sources across decisions and planner answers.', actions: [{ kind: 'record_answers', answerSources: [{ answerSourceKey: 'auth-part-1', sourceGroupKey: 'auth-answer', answer: 'Use Bun-native auth.' }, { answerSourceKey: 'rollout-part-1', sourceGroupKey: 'rollout-answer', answer: 'Use a staged rollout.' }, { answerSourceKey: 'auth-part-2', sourceGroupKey: 'auth-answer', answer: 'Add SSO for enterprise customers after the initial launch.' }, { answerSourceKey: 'pilot-part-1', sourceGroupKey: 'pilot-scope-answer', answer: 'Start with five enterprise customers before broader rollout.' }, { answerSourceKey: 'pilot-part-2', sourceGroupKey: 'pilot-scope-answer', answer: 'Keep early support manageable.' }], answers: [{ decisionKey: 'auth-strategy', summary: 'Choose the auth strategy', answerSourceGroupKey: 'auth-answer' }, { decisionKey: 'rollout-strategy', summary: 'Choose the rollout strategy', answerSourceGroupKey: 'rollout-answer' }], followThrough: { kind: 'planning_batch', groupKey: 'auth-rollout-follow-through', answers: [{ summary: 'Pilot scope', answerSourceGroupKey: 'pilot-scope-answer' }], requests: [{ taskKey: 'goal-docs', title: 'Capture auth rollout goal context', description: 'Record the auth and rollout answers across Goal docs.', acceptanceCriteria: ['The auth and rollout answers are durable.'], requestedUpdates: ['goal.md', 'design.md', 'notes/rollout.md'] }, { taskKey: 'task-graph', title: 'Decompose auth rollout task graph', description: 'Reflect the auth rollout answers in todo.yml.', acceptanceCriteria: ['The auth rollout task graph is visible in todo.yml.'], requestedUpdates: ['todo.yml'], blockedByTaskKeys: ['goal-docs'] }] } }] })); console.log('assistant finished')",
          '${PROMPT_FILE}',
          '${OUTCOME_FILE}',
        ],
        cwdMode: 'root',
    ...
    const server = startServer(undefined, workspaceRoot)
    const response = await postJson(server, '/api/goals/test/assistant/run', {
      content: 'Use grouped named answer sources across decisions and planner answers.',
    })

    expect(response.status).toBe(200)
    await expect(response.json()).resolves.toMatchObject({
      message: 'I reused grouped named answer sources across decisions and planner answers.',
      actionResults: expect.arrayContaining([
        expect.objectContaining({
          kind: 'record_answers',
          decisionKeys: ['auth-strategy', 'rollout-strategy'],
  ...
  test('runs the configured Goal assistant and mixes new durable decision topics with inferred planner answers from explicit answer-source route metadata', async () => {
    const workspaceRoot = await initGitRepo(rootDir())
    await writeAdapterConfig(workspaceRoot, {
      version: 1,
      assistant: {
        cmd: [
          'bun',
          '-e',
          "const [promptFile, outcomeFile] = process.argv.slice(1); const prompt = await Bun.file(promptFile).text(); if (!prompt.includes('Resolve the open auth decision and route the remaining reusable answer sources by explicit route metadata into one new launch decision plus one rollback planner answer.')) throw new Error('missing user message'); await Bun.write(outcomeFile, JSON.stringify({ message: 'I resolved the open auth decision and routed the remaining reusable answer sources by explicit route metadata into one new launch decision plus one rollback planner answer.', actions: [{ kind: 'record_answers', answerSources: [{ answerSourceKey: 'auth-strategy-answer', answer: 'Use Bun-native auth.' }, { answerSourceKey: 'pilot-scope-answer', answerKey: 'pilot-scope', answer: 'Start with five enterprise customers before broader launch.' }, { answerSourceKey: 'launch-shape-part-1', route: 'decision', summaryKey: 'launch-shape', answer: 'Use a staged rollout.' }, { answerSourceKey: 'launch-shape-part-2', route: 'decision', summaryKey: 'launch-shape', answer: 'Keep the launch reversible.' }, { answerSourceKey: 'rollback-trigger-part-1', route: 'planning', summaryKey: 'rollback-trigger', summary: 'Rollback trigger', answer: 'Abort after two regressions.' }, { answerSourceKey: 'rollback-trigger-part-2', route: 'planning', summaryKey: 'rollback-trigger', answer: 'Pause launch until fixes ship.' }], sourceResponseFormat: 'pending_answer_sources', inferOpenDecisions: true, inferDecisionTopics: true, followThrough: { kind: 'planning', title: 'Capture rollout notes', description: 'Record rollout details before more planning work continues.', acceptanceCriteria: ['Rollout notes are durable.'], answers: [{ summary: 'Pilot scope', answerKey: 'pilot-scope' }], inferRemainingAnswers: true, requestedUpdates: ['goal.md', 'notes/rollout.md'] } }] })); console.log('assistant finished')",
          '${PROMPT_FILE}',
          '${OUTCOME_FILE}',
        ],
        cwdMode: 'root',
    ...
    const response = await postJson(server, '/api/goals/test/assistant/run', {
      content:
        'Resolve the open auth decision and route the remaining reusable answer sources by explicit route metadata into one new launch decision plus one rollback planner answer.',
    })

    expect(response.status).toBe(200)
    await expect(response.json()).resolves.toMatchObject({
      message:
        'I resolved the open auth decision and routed the remaining reusable answer sources by explicit route metadata into one new launch decision plus one rollback planner answer.',
      actionResults: expect.arrayContaining([
        expect.objectContaining({
          kind: 'record_answers',
          decisionKeys: ['auth-strategy', 'D-1'],
