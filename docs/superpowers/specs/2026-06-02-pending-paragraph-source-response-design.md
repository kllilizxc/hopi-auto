# Pending Paragraph Source Response Design
...
Let one less-structured shared reply deterministically materialize onto more than one already-known pending answer consumer without repeating question/topic anchors, ordered-list markers, or clause-level splitting.
...
- the user may answer them in ordinary separate paragraphs
- there may be no repeated question text, no repeated topic labels, no ordered-list markers, and no useful clause splitting
...
`pending_paragraphs` treats the shared reply as a sequence of blank-line-separated answer paragraphs.

Those paragraphs are assigned in deterministic current pending order.

### Supported Pending Consumers

`pending_paragraphs` now works for:
...
Runtime consumes pending paragraphs in the same stable order already implied by the calling surface:
...
Each pending consumer receives the next unresolved paragraph.

If no paragraph remains for a pending consumer, runtime fails deterministically.

## Non-Goals

- inferring brand-new durable decision topics from unlabeled paragraph replies
- inferring planner-only remaining answers from unlabeled paragraph replies
- semantic grouping of multiple paragraphs into one later answer block
- replacing stronger question/topic anchored formats when the reply already exposes more authority

## Acceptance Criteria

- assistant and Bun API can resolve more than one current open decision from one less-structured shared paragraph reply without repeating question/topic anchors
- direct planning request surfaces can materialize more than one explicit planner answer from the same paragraph reply
- the same shared runtime path powers decision-backed and direct-planning interpretation
- runtime rejects the surface deterministically when pending consumers outnumber the available paragraphs
