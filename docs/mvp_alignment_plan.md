...
| Goal delivery | new Goal clarifies only material ambiguity, records design, builds, reviews, integrates, and completes | CardGame theme and Preview Goals both reached `done` through Planner, Generator, Reviewer, C1, and final Planner assessment; `goalDelivery.test.ts` now repeats the public Inbox-to-C1 skeleton with deterministic runners and zero model calls |
