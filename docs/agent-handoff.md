...
- richer mixed answer workflows where one user reply should atomically resolve some durable decision topics while also capturing other non-decision answers directly into planning follow-through
...
1. Extend Goal assistant and planner/runtime behavior so one user reply can mix decision-backed answer capture with non-decision answer-backed planning follow-through atomically where only part of the reply deserves a durable decision topic.
2. Add richer planner/runtime workflows on top of Goal-local durable docs, `planning-requests.yml`, and the current deterministic scheduler core, now that planning follow-through carries explicit decision lineage, captured non-decision answers, generalized requested-update paths, scheduler-enforced coverage checks, automatic decision-to-request enrichment, Goal-doc-aware coverage policy, grouped multi-task follow-through, grouped decision-lineage propagation, durable grouped task keys for incremental extension, and current-open-leaf blocker propagation for grouped planning.
