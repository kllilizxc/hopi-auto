...
- Question-derived `inferDecisionTopics` and `inferRemainingAnswers` now also preserve the original question text as durable `prompt` authority on newly inferred decision topics and planner captured answers, so `question_blocks` / `question_spans` / `question_middle_spans` / `question_closing_spans` / `question_closing_blocks` / `question_middle_blocks` no longer lose the canonical user-facing question when runtime materializes those items automatically.
