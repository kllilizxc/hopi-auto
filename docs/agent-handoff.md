...
- Answer-driven `inferDecisionTopics` now also supports `topic_sentences`, `topic_spans`, `topic_closing_spans`, and `topic_paragraphs`, so remaining topic-bearing sentence surfaces can become new durable decision topics without relying on inline labels, labeled sections, ordered structures, or explicit `answers[]` topic mapping.
...
- Question-derived `inferDecisionTopics` and `inferRemainingAnswers` now also preserve the original question text as durable `prompt` authority on newly inferred decision topics and planner captured answers, so `question_blocks` / `question_spans` no longer lose the canonical user-facing question when runtime materializes those items automatically.
