...
- Answer-driven `inferDecisionTopics` now also supports `topic_sentences` and `topic_paragraphs`, so remaining topic-bearing sentences and paragraphs can become new durable decision topics without relying on inline labels, labeled sections, ordered structures, or explicit `answers[]` topic mapping.
