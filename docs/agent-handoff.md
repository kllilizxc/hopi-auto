...
- Answer-driven and direct planning assistant/API surfaces now also support `sourceResponseFormat: "auto"`, so callers can let shared runtime deterministically choose the strongest successful existing interpretation surface by fixed authority priority instead of hard-coding `question_spans`, `topic_paragraphs`, `matching_answer_sources`, or one weaker pending-order fallback up front; when no existing deterministic surface fits, runtime still fails closed.
