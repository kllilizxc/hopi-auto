...
- Answer-driven `inferDecisionTopics` now also supports `topic_sentences`, `topic_spans`, and `topic_paragraphs`, so remaining topic-bearing sentence surfaces can become new durable decision topics without relying on inline labels, labeled sections, ordered structures, or explicit `answers[]` topic mapping.
