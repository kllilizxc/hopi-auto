...
- assistant thread inspection now also surfaces structured `action` and `action_result` authority like requested mutation details and resolved interpretation format instead of flattening history to `actionType | summary` only
...
- Goal-scoped assistant thread storage now also persists structured assistant `action` and `action_result` payloads instead of only lossy summary entries
