...
- assistant thread inspection, bundled assistant context, and assistant run detail inspection now also surface grouped-planning request authority on direct `request_planning_batch` actions and decision follow-through `planning_batch` actions, so per-request update targets and request-to-request dependencies are visible instead of staying buried inside grouped request arrays
- assistant thread inspection, bundled assistant context, and assistant run detail inspection now also surface child-level workflow action metadata for `request_planning_workflows` and decision follow-through `workflow_batch`, instead of flattening those actions to workflow counts plus top-level reuse/shared-answer fields
- assistant thread inspection, bundled assistant context, and assistant run detail inspection now also surface workflow-child dependency authority on structured assistant `action` payloads, so `blockedByWorkflowKeys` is visible alongside each child instead of staying buried inside raw workflow action JSON
- assistant thread inspection, bundled assistant context, and assistant run detail inspection now also surface child-specific workflow action context like extra `decisionRefs` and child-local planner-answer counts, instead of making readers infer that authority from raw child payloads or root workflow metadata
...
- assistant thread inspection, bundled assistant context, and assistant run detail inspection now also surface child-level workflow result metadata for `request_planning_workflows` and decision follow-through `workflow_batch` instead of flattening those results to one workflow count plus aggregate request/task ids
...
- assistant run records now also persist transcript `toolInvocationKey` on assistant-side runtime events instead of silently stripping it during run-record schema validation, so assistant run API readback, bundled inspection, and UI event detail can preserve the same tool-correlation authority already present in normalized vendor transcript events
