...
- Assistant-thread history now also persists structured `action_result` authority instead of only lossy `actionType + summary`, and both Bun thread inspection plus bundled assistant context now reuse that same structured result to surface resolved interpretation formats and follow-through metadata without opening raw run records.
...
- assistant thread inspection now also surfaces structured `action_result` authority like resolved interpretation format instead of flattening history to `actionType | summary` only
...
- Goal-scoped assistant thread storage now also persists structured assistant `action_result` payloads instead of only lossy summary entries
