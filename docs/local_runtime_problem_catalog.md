...
| H-45 | Work 新增 `contextRefs`、`ownerMessages` 后仍沿用 epoch 1；历史 Work 不能解析，`/api/state` 又把校验失败投影成空 Goal 列表。旧 Attempt 的 `result: attention` 同时被新版枚举反复报为 corrupt。 | 持久化 schema 发生不兼容变化，却既没有提升 epoch，也没有在停机边界完成数据替换；API 还把错误日志当成了空业务状态。 | epoch 2 使用一次性离线迁移：精确备份被改文件，为 Work 补显式空集合，将旧 `attention` outcome 归一为 `fail` 并保留 `application: attention`，最后切换 Home epoch。运行时不保留旧格式分支；Project state 显式返回 `validationError`。 | `domain/project.ts`、`scripts/migrate-home-schema-2.ts`、`api/statePresenter.ts` |
| H-46 | `reset:project` 删除被 `.gitignore` 覆盖的历史 Goal 后，`git add -A -- .hopi/docs/goals` 被 Git 拒绝，reset 停在 release 更新之前并留下部分暂存删除。 | 删除已跟踪历史只需要更新 index，`-A` 却要求 Git重新处理 ignored path，把“记录删除”和“添加未知文件”混成了一个操作；ref 边界前也没有恢复原 index。 | reset 使用 `git add -u`，只暂存当前 release 已跟踪文件的删除；在 release ref 成功前发生任何错误都会恢复原 index tree。fixture 让 `.hopi/docs` 保持 ignored，覆盖真实约束。 | `runtime/projectReset.ts::removeGoalHistoryFromRelease`、`tests/projectReset.test.ts` |
| H-47 | Wake、Assistant action receipt、Preview、bootstrap cleanup 或 reset 的路径存在但不可读时，状态被静默投影为空；Home reset 的路径边界检查还会退回未经 canonicalize 的字符串。 | 文件系统读取把所有错误都当成“路径尚未创建”，混淆缺失与权限、路径类型或存储故障。 | 可选路径统一只把 `ENOENT` 视为缺失；其他错误直接传播，避免 Coordinator 从虚构的空状态继续，也避免 destructive reset 绕过真实路径边界检查。 | `storage/filesystem.ts` |
| H-48 | Project reset 清除了 ignored Goal 残留，但启动时仍从更早的 release 历史恢复 C1，导致已清空 Project 因“done Work 缺失”被阻塞。 | Git 提交同时承担审计历史与当前状态重建；reset 只在有受跟踪 Goal 文件时提交删除，也没有标记状态纪元。 | reset 现在写入 `HOPI-Project-Reset` release epoch，即使 tree 不变也能形成边界；C1 重建只读取该 Project 最近 reset epoch 之后的记录。旧提交仍可审计，但不再恢复为当前状态。 | `runtime/projectReset.ts`、`runtime/c1Integrator.ts`、`tests/projectReset.test.ts` |
...
| H-34 | 同一 Work 的 task checkout、integration 和 release 曾不在同一 Project namespace，导致路径碰撞。 | worktree 和 release 当时不属于 Project × Repo binding。 | 当前路径统一为 `projects/<projectId>/…`，release 为 `hopi/project/<projectId>/release`。代码只读取当前 `schemaEpoch`；另一 epoch 必须停机后整体 reset，或执行该 epoch 唯一的离线替换迁移。 | `domain/project.ts`、`runtime/managedWorktreePaths.ts`、`runtime/homeReset.ts` |
